begin;
-- Run this whole file after the hourly-quota migrations. Safe to rerun.
-- Existing wins, saves and match history are not backfilled or rewritten.

-- Source: 20260913162021_short_wins_and_score_receipts.sql
...
-- Source: 20260913164006_preserve_lifetime_challenge_results.sql
...
-- Each row should show false / false / true; v2 should also show true in the last column.
select p.proname as function_name,
       position('TOO_FAST' in pg_get_functiondef(p.oid)) > 0 as hourly_cap_remaining,
       position('TOO_SHORT' in pg_get_functiondef(p.oid)) > 0 as minimum_duration_remaining,
       to_regprocedure('public.sr_submit_once(text,text,uuid,integer,jsonb)') is not null as receipts_installed,
       case when p.proname='sr_submit_v2' then
         position('challenge_ineligible_reason' in pg_get_functiondef(p.oid)) > 0
       else null end as challenge_preserves_lifetime_wins
